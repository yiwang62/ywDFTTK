...
*/
	...
	*/
        void solveDMev(FILE *fexact, ATM **atomP, int natomP, int calcNA, double *BCharg,
		double *ILOTO, double *IRed, RAMAN *IRaman, double *RRvv) {
            double b[3];
            mproduct(vecq, qvec, b);
            for (int j=0; j<3; j++) b[j] /= PI2;
            fprintf (fexact, "\n\n Q= %9.6lf %9.6lf %9.6lf, q= %9.6lf %9.6lf %9.6lf\n\n",  b[0], b[1], b[2], vecq[0]/PI2, vecq[1]/PI2, vecq[2]/PI2);
	    fprintf(fexact, "                    ");
	...
	*/
	    ...
	    //gsl_eigen_symm_sort (eval, evec, GSL_EIGEN_SORT_ABS_ASC);
	    gsl_sort_vector(eval);
...
*/
	...
	LatDYN::SrealFij(Fij_Orig, natomS, energy_in_real_space);

	if (!silent) printf("\nSolving frequencies in real space :");
	if (!silent) cputim->elptime(stdout);

        double *energyG = new double[nTHz]; //phonons at Gamma point
        LatDYN::SrealFij(Sij, natomP, energyG);
	...
	*/
	for (i=-kx; i<kx; i++) {
		...
		if (exactK(a,Rcell)) {
                    ...
                    DM->solveDMev(fexact, atomP, natomP, calcNA, BCharg, ILOTO, IRed, IRaman, RRvv);
		    exactP++;
		    if (i==0 && j==0 && k==0) {
