...
int i, j;
	for (i=0; i<3; i++) {
		for (j=0; j<3; j++) {
			if (shortout) printf(" %13.7lf", a[i][j]);
			else printf(" %20.10lf", a[i][j]);
		}
		printf("\n");
	}
}
...
bool cartisian = false;
	...
	natomS = 0;
        ...
        while (i < argc) {
                ...
                else if (!strcmp(argv[i], "-AFM")) AFM = 0;
                ...
                else if (!strcmp(argv[i], "-short")) shortout = 1;
                else if (!strcmp(argv[i], "-outpMatrix")) outpMatrix = 0;
...
//w3(s);
	...
	for (int k=0; k<natomS;k++) {
		...
		if (!strcmp(sym, atomS[i]->sym)) {
		    ...
		    if (esym==1)
		      if (shortout) fprintf(ftmp, "%12.7lf %12.7lf %12.7lf %s\n", 
			v[0], v[1], v[2], atomS[i]->comment);
		      else fprintf(ftmp, "%15.10lf %15.10lf %15.10lf %s\n", 
			v[0], v[1], v[2], atomS[i]->comment);
		    else
		      if (shortout) fprintf(ftmp, "%12.7lf %12.7lf %12.7lf\n", v[0], v[1], v[2]);
		      else fprintf(ftmp, "%15.10lf %15.10lf %15.10lf\n", v[0], v[1], v[2]);
	...
	fclose(ftmp);
	    ...
	    if (cartisian) printf("%s\nC\n", natline);
            ...
            for (int i=0; i<nat->GetN(); i++) 
                printf(" %s",el->str(i));
                //for (int k=0; k<atoi(nat->str(i)); k++) printf(" %s",el->str(i));
            printf("\n");
            for (int i=0; i<nat->GetN(); i++) 
                printf(" %d",atoi(nat->str(i))*natomS/natom);
                //for (int k=0; k<atoi(nat->str(i)); k++) printf(" %d",atoi(nat->str(i))*natomS/natom);
            printf("\n");
	    if (cartisian) printf("C\n");
	    else printf("D\n");
        }
